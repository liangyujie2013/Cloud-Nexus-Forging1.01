...
> 所有 API、文件路径、数据结构、命令均来自 `//webapp` 工作区的实际文件。
...
### 1.1 项目定位
- **产品名**：Cloud Nexus Forging（CNF）—— 企业级 KVM 虚拟化管理平台，对标 VMware vCenter / Proxmox VE / SmartX CloudTower。
- **目标场景**：纳管真实 RHEL/Rocky 8/9 KVM 宿主机（libvirt），提供数据中心→集群→主机→虚拟机四级层级管理、存储/网络/GPU/高可用/监控/权限的统一 Web 控制台。
- **当前形态（真实）**：仓库 `//webapp` 内运行的是一套 **纯前端 Vue3 SPA + Hono Mock 后端**。**后端所有数据来自 `src/mock-data.ts` 内存数据，不连接任何真实 libvirt / 数据库**。它是一个**功能演示 / 交互原型（UI + 交互逻辑完整，数据为模拟）**。
- **另有一套 Go 源码工程**位于 `//webapp/cnf-source/`（Go 1.23 + Fiber v3 + libvirt + MySQL/PostgreSQL），是「真实虚拟化落地」的代码骨架，**与前端 Mock 后端是两条独立路径，前端当前不调用它**。
...
```env
# //webapp 本地开发（src/server-node.ts 唯一读取项）
PORT=3000
```
...
- [ ] **N8 告警指标目录下拉**：当前 `view-monitoring.js:345` 监控指标是**自由文本输入**（placeholder「如 host.cpu_usage」），需改为**全量指标下拉 + 描述 + VMware 术语** - 难度【中】（需新增指标目录数据/端点）
...
- [x] N1~N5 术语已校准（VMware 风格：已连接/资源池/SR-IOV 等）
...
| 角色 | 路径 |
|------|------|
| 后端路由+API | `//webapp/src/index.tsx` |
| Mock 数据/类型 | `//webapp/src/mock-data.ts` |
| 本地启动器 | `//webapp/src/server-node.ts` |
| 全局 api()/右键菜单 | `//webapp/public/static/component-context-menu.js`（`window.api`、`window.API_BASE='/api/v1'`、`window.useContextMenu()`） |
| 拓扑 Store | `//webapp/public/static/store-topology.js`（`window.cnfTopology`） |
| SPA 壳/路由 | `//webapp/public/static/app.js`（`window.__CNF_VIEWS` 注册表） |
| i18n | `//webapp/public/static/i18n.js`（`window.t`） |
...
### 4.1 前端/Mock 路径（当前运行）
- **无数据库**。所有数据在 `//webapp/src/mock-data.ts` 内存对象 `mockData`，进程重启即重置。
...
### 6.1 必须复制 vs 必须排除
**必须复制**：整个 `//webapp` 除排除项外的全部源码 —— `src/`、`public/`、`scripts/`、`cnf-source/`（除 bin/dist）、`package.json`、`package-lock.json`、`*.config.*`、`README.md`、`P-FIXES.md`、`CNF_Handover.md`、`.gitignore`。
...
# 2. 取代码
git clone <REPO_URL> webapp && cd webapp
...
| N8 告警指标目录下拉 | 当前自由文本易错，需 VMware 术语指标目录 | 新增指标目录数据 | `view-monitoring.js:345`、`mock-data.ts`、`i18n.js` |
...
项目事实（务必先认知）：
- 工作区 //webapp 是「Vue3 SPA（public/static/*.js，无构建步骤）+ Hono Mock 后端（src/index.tsx，数据全在 src/mock-data.ts 内存）」。
- 本地只能用 `export PATH=/opt/node18/bin:$PATH && cd //webapp && npm run dev:node`（端口 3000）。`npm run dev`(wrangler) 因 glibc 版本无法运行。
- 另有 //webapp/cnf-source 是独立 Go 真实虚拟化骨架，前端当前不调用它。
