...
const dict = {
  zh: {
    ...
    hw_check_libvirt: '虚拟化计算组件（libvirt/KVM）', hw_check_tcp: 'libvirt 远程 TCP 监听',
    ...
    // ===== 主机管理 v2.0 (zh) =====
    nav_mod_hosts: '主机管理', nav_hosts_list: '主机列表', nav_hosts_management: '主机管理', nav_hosts_network: '主机网络', nav_hosts_detail: '主机详情',
    ...
    hv_list_title: '主机列表', hv_list_intro: '全集群宿主机实时负载（CPU / 内存 / 磁盘）。点击卡片进入单台主机详情与运维。',
    hv_mgmt_title: '主机管理', hv_mgmt_intro: '宿主机运维：实时状态、电源、维护模式、连接管理。每台主机一行，点击展开运维操作。',
    hv_net_title: '主机网络', hv_net_intro: '宿主机网卡管理：真实读取网卡（名称/MAC/模式/IP/掩码/网关/DNS），支持 DHCP↔静态切换并写配置生效。',
    hv_metric_loading: '正在采集实时指标…', hv_metric_unreachable: '不可达 / 未保存凭据', hv_metric_na: '暂无数据',
    hv_col_cpu: 'CPU 使用率', hv_col_mem: '内存使用率', hv_col_disk: '根分区', hv_col_load: '负载', hv_col_uptime: '运行时长',
    hv_refresh: '刷新指标', hv_open_detail: '查看详情', hv_open_net: '配置网卡', hv_view_net: '查看网卡',
    hv_no_hosts: '尚未纳管任何宿主机', hv_uptime_d: '天', hv_uptime_h: '时', hv_uptime_m: '分',
    hd_kernel: '内核', hd_services: '服务与运维状态（实时）', hd_module: '模块', hd_loaded: '已加载', hd_not_loaded: '未加载',
    hd_port: '端口', hd_boot_time: '启动时刻', hd_virt_caps: '虚拟化能力（libvirt 实时探测）', hd_threads_core: '每核线程',
    hd_probe_ok: 'libvirt 实时探测成功', hd_probe_fail: 'libvirt 探测失败（显示纳管时采集的清单）',
    hd_no_data: '无数据', hd_no_inventory: '暂无硬件清单（纳管时未采集到）', hd_realtime_note: '每 3 秒采集一次真实指标',
  ...
  en: {
    ...
    hw_check_libvirt: 'Virtualization components (libvirt/KVM)', hw_check_tcp: 'libvirt remote TCP listener',
    ...
    nav_mod_hosts: 'Host Management', nav_hosts_list: 'Host List', nav_hosts_management: 'Host Ops', nav_hosts_network: 'Host Network', nav_hosts_detail: 'Host Detail',
    ...
    hv_list_title: 'Host List', hv_list_intro: 'Real-time load (CPU / memory / disk) across all clustered hosts. Click a card for host detail & ops.',
    hv_mgmt_title: 'Host Operations', hv_mgmt_intro: 'Host ops: live status, power, maintenance mode, connection management. One host per row.',
    hv_net_title: 'Host Network', hv_net_intro: 'Host NIC management: read real NICs (name/MAC/mode/IP/mask/gateway/DNS), switch DHCP↔static and apply.',
    hv_metric_loading: 'Collecting live metrics…', hv_metric_unreachable: 'Unreachable / no saved credential', hv_metric_na: 'No data',
    hv_col_cpu: 'CPU Usage', hv_col_mem: 'Memory Usage', hv_col_disk: 'Root Disk', hv_col_load: 'Load', hv_col_uptime: 'Uptime',
    hv_refresh: 'Refresh metrics', hv_open_detail: 'View detail', hv_open_net: 'Configure NIC', hv_view_net: 'View NICs',
    hv_no_hosts: 'No hosts onboarded yet', hv_uptime_d: 'd', hv_uptime_h: 'h', hv_uptime_m: 'm',
    hd_kernel: 'Kernel', hd_services: 'Services & Ops State (live)', hd_module: 'module', hd_loaded: 'loaded', hd_not_loaded: 'not loaded',
    hd_port: 'Port', hd_boot_time: 'Boot time', hd_virt_caps: 'Virtualization Capabilities (libvirt live probe)', hd_threads_core: 'Threads/core',
    hd_probe_ok: 'libvirt live probe OK', hd_probe_fail: 'libvirt probe failed (showing inventory from onboarding)',
    hd_no_data: 'No data', hd_no_inventory: 'No hardware inventory (not collected at onboarding)', hd_realtime_note: 'Real metrics sampled every 3s',
