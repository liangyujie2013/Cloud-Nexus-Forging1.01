...
const dict = {
  zh: {
    ...
    hw_check_libvirt: 'libvirt 安装状态', hw_check_tcp: 'libvirtd TCP 监听',
    ...
    hw_check_libvirt_no: '未安装（需手工安装或开启自动安装）', hw_check_libvirt_autoinstall: '未安装 · 纳管时将自动安装',
    hw_check_tcp_ok: 'TCP {port} 已监听', hw_check_tcp_no: '未监听（需开启自动安装或手工配置）', hw_check_tcp_autoinstall: '未监听 · 纳管时将自动配置',
    hw_check_mem_unknown: '硬件信息采集中',
    hw_auto_install: '自动安装 libvirt 和 KVM（若未安装）', 
    hw_auto_install_desc: '裸机或未装虚拟化组件的主机：系统将通过 SSH 自动安装 qemu-kvm / libvirt，并配置 libvirtd TCP（16509）后纳管。需 root 或免密 sudo。仅支持 RHEL 8/9/10 及衍生版（Rocky/AlmaLinux/CentOS Stream，dnf）。',
    hw_precheck_blocked: 'CPU 虚拟化未通过，或 libvirt/TCP 缺失且未开启「自动安装」。请开启自动安装开关，或先在目标主机手工安装配置。',
    hw_recheck: '重新预检',
    hw_dep_installing: '正在通过 SSH 自动安装 libvirt + KVM 并配置 TCP…', hw_dep_onboarding: '正在采集硬件、落库并以 qemu+tcp 验证连接…',
    hw_dep_ready: '点击「开始纳管」执行节点纳管', hw_dep_ready_auto: '点击「开始纳管」：将自动安装 libvirt+KVM（若缺失）后纳管',
    hw_dep_success: '主机 {host} 已成功加入集群 {cluster}', hw_dep_failed: '纳管失败，请查看下方日志', 
    hw_install_log: '安装与配置日志（真实执行）', hw_log_live: '实时',
    hw_offline_ready: '平台离线安装包已就绪', hw_offline_pkgs: '个包', hw_offline_fallback: '目标主机源不可用时自动推送本地安装',
    hw_offline_empty: '平台未预置离线安装包，目标主机需能访问 yum/dnf 在线源',
  ...
  en: {
    ...
    hw_check_libvirt: 'libvirt status', hw_check_tcp: 'libvirtd TCP listener',
    ...
    hw_check_libvirt_no: 'Not installed (manual install or enable auto-install)', hw_check_libvirt_autoinstall: 'Not installed · will auto-install on onboarding',
    hw_check_tcp_ok: 'TCP {port} listening', hw_check_tcp_no: 'Not listening (enable auto-install or configure manually)', hw_check_tcp_autoinstall: 'Not listening · will auto-configure on onboarding',
    hw_check_mem_unknown: 'Collecting hardware info',
    hw_auto_install: 'Auto-install libvirt and KVM (if missing)',
    hw_auto_install_desc: 'For bare-metal hosts without virtualization: the system will install qemu-kvm / libvirt over SSH and configure libvirtd TCP (16509) before onboarding. Requires root or passwordless sudo. Supports RHEL 8/9/10 and derivatives (Rocky/AlmaLinux/CentOS Stream, dnf).',
    hw_precheck_blocked: 'CPU virtualization failed, or libvirt/TCP missing while auto-install is off. Enable auto-install, or install/configure on the target host first.',
    hw_recheck: 'Re-check',
    hw_dep_installing: 'Installing libvirt + KVM over SSH and configuring TCP…', hw_dep_onboarding: 'Collecting hardware, persisting and verifying via qemu+tcp…',
    hw_dep_ready: 'Click "Start Onboarding" to onboard the node', hw_dep_ready_auto: 'Click "Start Onboarding": will auto-install libvirt+KVM (if missing) then onboard',
    ...
    hw_offline_ready: 'Platform offline packages ready', hw_offline_pkgs: 'packages', hw_offline_fallback: 'auto-pushed for local install when target repos are unavailable',
    hw_offline_empty: 'No offline packages on platform; target host must reach yum/dnf online repos',
