...
const dict = {
  zh: {
    ...
    topo_collapse: '收起', topo_expand: '展开', topo_open_detail: '查看详情',
    topo_expand_all: '全部展开', topo_collapse_all: '全部收起',
    topo_belongs: '归属',
    // ===== 添加主机向导（节点纳管）=====
    hw_title: '添加主机到集群',
    ...
    wiz_pin_auto_desc: '启用后，系统将从所选 NUMA 节点自动连续分配独享物理核给各 vCPU（按业界最佳实践自动分配，无需手动点格子）。',
    ...
    swd_open: '标准交换机', swd_title: '标准交换机（Linux Bridge + Bond）',
    swd_desc: '在宿主机上以 nmcli 创建标准虚拟交换机：单网卡直连网桥；多网卡先组 bond（默认 active-backup 主备容错）再桥接。',
    swd_existing: '已有交换机', swd_no_switch: '该主机暂无标准交换机',
    swd_free_nics: '可用物理网卡', swd_no_free: '没有可用的空闲物理网卡',
    swd_pick_uplink: '点击网卡选择上联口（多选自动组 bond）',
    swd_mgmt_nic: '管理网卡', swd_mgmt_warn: '该网卡承载管理/默认路由，纳入交换机有断网风险，需勾选确认。',
    swd_ack_mgmt: '我已知晓风险，确认使用管理网卡',
    swd_sw_name: '交换机名称（网桥）', swd_bond_mode: 'Bond 模式',
    swd_ip_mode: 'IP 配置', swd_ip_none: '不配置', swd_ip_dhcp: 'DHCP 自动', swd_ip_static: '静态',
    swd_ipv4: 'IPv4 地址', swd_prefix: '前缀', swd_gateway: '网关', swd_dns: 'DNS',
    swd_create: '创建交换机', swd_delete: '删除', swd_uplinks: '上联口', swd_state: '状态',
    swd_unreachable: '主机不可达或缺少凭据，无法读取网卡。',
    swd_no_nm: '该主机未启用 NetworkManager（nmcli），无法管理标准交换机。',
    swd_created: '标准交换机创建成功', swd_deleted: '标准交换机已删除',
    swd_del_confirm: '确认删除该标准交换机？相关网桥/bond 连接将被移除。',
    swd_need_uplink: '请至少选择一块上联网卡',
    gsw_btn: '全局交换机',
    gsw_title: '全局交换机 · 多主机一致下发',
    gsw_desc: '定义一份标准交换机规格（网桥名 / bond 模式 / IP 模式），勾选一组主机并为每台选择上行物理网卡，平台逐主机真实下发；下方可一键核对各主机落地与一致性。',
    gsw_hosts: '目标主机',
    gsw_mode_ab: '主备',
    gsw_ip_none: '不配 IP（纯转发，推荐）',
    gsw_need_host: '请至少勾选一台目标主机',
    gsw_need_uplink_each: '每台勾选的主机都需至少选择一块上行网卡',
    gsw_mgmt_warn: '所选上行含管理网卡，下发可能中断该主机连接',
    gsw_ack_mgmt: '我已知晓：该主机上行含管理网卡，可能临时断连，仍要下发',
    gsw_no_free_nic: '该主机暂无可用空闲网卡（或不可达）',
    gsw_apply: '下发',
    gsw_delete: '删除',
    gsw_check: '核对一致性',
    gsw_apply_done: '下发完成：{ok}/{total} 台成功',
    gsw_del_done: '删除完成：{ok}/{total} 台成功',
    gsw_del_confirm: '确认在所选主机上删除全局交换机「{name}」？此操作会拆除网桥/bond。',
    gsw_present: '已存在',
    gsw_absent: '不存在',
    gsw_is_mgmt: '承载管理流量',
    gsw_consistency: '交换机「{name}」：{present}/{total} 台主机已落地',
    gsw_all_consistent: '全部一致 ✓',
    gsw_inconsistent: '存在差异，请检查未落地主机',
    vmk_open: '服务接口',
    vmk_title: '服务接口 · 流量标签',
    vmk_desc: '服务接口是主机级、带 IP 的网络接口，用流量标签区分各类服务网络（管理/迁移/存储/容错/复制/置备）。落地为带 IP 的网桥 + firewalld 区域标注；可选择上行口或仅本机 IP。',
    vmk_existing: '已有服务接口',
    vmk_none: '该主机暂无服务接口',
    vmk_new: '新建服务接口',
    vmk_all_used: '六种流量标签均已创建（同一主机每种标签唯一）',
    vmk_name: '名称',
    vmk_role: '流量标签',
    vmk_ip: 'IP',
    vmk_uplink: '上行',
    ...
    vmk_prefix: '前缀长度',
    vmk_gateway: '网关',
    vmk_no_uplink: '不绑上行（仅本机 IP）',
    vmk_is_mgmt: '承载管理流量',
    vmk_mgmt_warn: '该服务接口涉及管理网络，操作可能影响主机连接',
    vmk_ack_mgmt: '我已知晓：涉及管理网络，可能临时断连，仍要执行',
    vmk_prefix_invalid: '前缀长度需在 1..32',
    vmk_create: '创建服务接口',
    vmk_delete: '删除',
    vmk_created: '服务接口创建成功',
    vmk_deleted: '服务接口已删除',
    vmk_del_confirm: '确认删除服务接口「{name}」？将拆除其网桥与上行口。',
    // ===== 第6点 · VLAN（access 端口组 + trunk 中继）=====
    vld_open: 'VLAN', vld_title: 'VLAN 管理（端口组 + 中继）',
    vld_desc: '基于标准交换机配置 VLAN：access 端口组在父设备上建 VLAN 子接口并桥接到专属转发网桥（出口打 tag、入口剥 tag）；trunk 中继在网桥上启用 VLAN 过滤并放行一组 VLAN 透传。全部经 nmcli/bridge 真实下发。',
    vld_access_existing: '已有 access 端口组', vld_trunk_existing: '已启用 VLAN 过滤的网桥（trunk）',
    vld_port_name: '端口组网桥', vld_vlan_id: 'VLAN ID', vld_parent: '父设备',
    vld_allowed_vlans: '放行 VLAN', vld_no_access: '尚无 access 端口组', vld_no_trunk: '尚无 trunk 网桥',
    vld_new: '新建 VLAN', vld_mode_access: 'access 端口组', vld_mode_trunk: 'trunk 中继',
    vld_access_hint: '在某个父设备（bond/网桥）上创建一个指定 VLAN 的接入端口组，挂到此端口组的虚机即处于该 VLAN。',
    vld_trunk_hint: '在网桥上启用 VLAN 过滤并放行一组 VLAN，使带 tag 的帧透传（中继）；虚机网卡再各自指定 access VLAN。',
    vld_bridge_name: '端口组网桥名', vld_optional: '可选', vld_no_parent: '尚无可用父设备，请先创建标准交换机',
    vld_trunk_bridge: '目标网桥', vld_vlans_hint: '逗号分隔，支持范围',
    vld_create: '创建端口组', vld_set_trunk: '应用中继', vld_delete: '删除',
    vld_mgmt_warn: '该设备承载管理流量，VLAN 改动可能影响管理网络（可能断开 SSH）。',
    vld_ack_mgmt: '我已知晓风险，继续操作',
    vld_vlan_invalid: 'VLAN ID 非法（合法范围 1-4094）',
    vld_created: 'access VLAN 端口组创建成功', vld_deleted: 'VLAN 端口组已删除',
    vld_trunk_done: 'trunk（VLAN 中继）配置成功', vld_del_confirm: '确认删除该 VLAN 端口组？',
    ...
    fw_short: '防火墙', fw_manage: '管理防火墙', fw_title: '防火墙管理（firewalld）', fw_loading: '正在读取防火墙状态…',
    fw_load_fail: '无法读取防火墙状态（凭据缺失或主机不可达）', fw_not_installed: '目标主机未安装 firewalld，无法管理防火墙',
    fw_status: '防火墙状态', fw_on: '已开启', fw_off: '已关闭', fw_turn_on: '开启防火墙', fw_turn_off: '关闭防火墙',
    fw_platform: '平台端口', fw_ready: '已就绪', fw_not_ready: '未就绪', fw_missing: '缺失端口', fw_open_platform: '一键放行平台端口',
    fw_open_ports: '已放行端口', fw_no_ports: '暂无放行端口', fw_services: '放行服务', fw_remove: '移除',
    fw_add_custom: '添加自定义端口策略', fw_port_ph: '如 8080 或 6000-6010', fw_add: '添加', fw_port_invalid: '端口格式非法（如 8080 或 6000-6010）',
    fw_op_done: '防火墙操作已生效', fw_exec_steps: '已执行命令', fw_batch_btn: '批量防火墙', fw_batch_title: '多机批量防火墙',
    fw_batch_hint: '勾选主机并选择操作，将并发对各主机真实执行（逐机返回结果）', fw_batch_action: '批量操作', fw_batch_hosts: '选择主机',
    fw_act_open_platform: '放行平台必需端口', fw_act_enable: '开启防火墙(并放行平台端口)', fw_act_disable: '关闭防火墙',
    fw_act_add: '添加端口', fw_act_remove: '移除端口', fw_batch_run: '执行批量操作', fw_batch_pick: '请至少勾选一台主机',
    fw_batch_done: '批量完成：{ok}/{total} 成功', fw_batch_results: '执行结果', fw_batch_results_n: '结果',
    se_short: 'SELinux', se_manage: '管理 SELinux', se_title: 'SELinux 管理', se_loading: '正在读取 SELinux 状态…',
    se_load_fail: '无法读取 SELinux 状态（凭据缺失或主机不可达）', se_not_avail: '目标主机不支持 SELinux',
    se_current: '运行时', se_persistent: '持久配置', se_inconsistent: '运行时与持久配置不一致（重启后将以持久配置为准）',
    se_set_mode: '设置模式', se_reboot_note: '注意：enforcing↔permissive 即时生效；切换到/从 disabled 需重启主机才完全生效（不会伪造即时生效）',
    se_need_reboot: '该变更需重启主机后才完全生效', se_need_reboot_short: '需重启', se_apply: '应用', se_op_done: 'SELinux 已设置',
    se_batch_btn: '批量 SELinux', se_batch_title: '多机批量 SELinux', se_batch_hint: '勾选主机并选择模式，将并发真实执行（逐机返回结果，需重启者会标注）',
    se_batch_run: '执行批量设置', se_batch_pick: '请至少勾选一台主机', se_batch_done: '批量完成：{ok}/{total} 成功，{reboot} 台需重启',
    sp_short: 'SSH端口', sp_manage: '修改 SSH 端口', sp_title: 'SSH 端口修改', sp_batch_btn: '批量改 SSH 端口', sp_batch_title: '批量修改 SSH 端口',
    sp_risk: '高风险操作：改错可能导致无法 SSH 登录该主机。平台采用「新旧端口并存→新端口验证通过→才切换并同步数据库」策略，验证失败将自动保留旧端口，主机不会失联。',
    sp_batch_risk: '高风险批量操作：将对所选主机逐台修改 sshd 端口。每台均先并存验证，验证失败者自动保留旧端口、不影响其他主机。',
    sp_current: '当前端口', sp_new_port: '新端口 (1-65535)', sp_safe_note: '将自动联动：SELinux 端口标签 + firewalld 放行 + sshd_config 双写校验 + 重启 sshd + 验证连通后同步数据库。',
    sp_ack: '我已了解风险，确认执行端口修改', sp_need_ack: '请先勾选确认风险', sp_invalid: '端口须为 1-65535 的整数',
    sp_apply: '修改并验证', sp_verified: '新端口已验证连通，数据库已同步', sp_verify_fail: '新端口验证失败，已保留旧端口（数据库未变更）', sp_op_done: 'SSH 端口操作完成',
    sp_batch_run: '执行批量修改', sp_batch_pick: '请至少勾选一台主机', sp_batch_done: '批量完成：{ok}/{total} 成功并验证',
  ...
  en: {
    ...
    topo_collapse: 'Collapse', topo_expand: 'Expand', topo_open_detail: 'Open detail',
    topo_expand_all: 'Expand all', topo_collapse_all: 'Collapse all',
    ...
    wiz_pin_auto_desc: 'When enabled, dedicated cores are auto-allocated contiguously from the chosen NUMA node (industry best practice — no manual grid).',
    ...
    swd_open: 'Standard Switch', swd_title: 'Standard Switch (Linux Bridge + Bond)',
    swd_desc: 'Create a standard virtual switch on the host via nmcli: single NIC attaches directly to a bridge; multiple NICs form a bond (default active-backup) then bridge.',
    swd_existing: 'Existing switches', swd_no_switch: 'No standard switch on this host',
    swd_free_nics: 'Free physical NICs', swd_no_free: 'No free physical NICs available',
    swd_pick_uplink: 'Click NICs to pick uplinks (multi-select auto-forms a bond)',
    swd_mgmt_nic: 'Mgmt NIC', swd_mgmt_warn: 'This NIC carries the management/default route; including it risks losing connectivity and requires confirmation.',
    swd_ack_mgmt: 'I understand the risk, use the management NIC',
    swd_sw_name: 'Switch name (bridge)', swd_bond_mode: 'Bond mode',
    swd_ip_mode: 'IP config', swd_ip_none: 'None', swd_ip_dhcp: 'DHCP', swd_ip_static: 'Static',
    swd_ipv4: 'IPv4 address', swd_prefix: 'Prefix', swd_gateway: 'Gateway', swd_dns: 'DNS',
    swd_create: 'Create switch', swd_delete: 'Delete', swd_uplinks: 'Uplinks', swd_state: 'State',
    swd_unreachable: 'Host unreachable or missing credentials; cannot read NICs.',
    swd_no_nm: 'NetworkManager (nmcli) is not enabled on this host; cannot manage standard switches.',
    swd_created: 'Standard switch created', swd_deleted: 'Standard switch deleted',
    swd_del_confirm: 'Delete this standard switch? Related bridge/bond connections will be removed.',
    swd_need_uplink: 'Pick at least one uplink NIC',
    gsw_btn: 'Global Switch',
    gsw_title: 'Global Switch · Consistent Multi-Host Rollout',
    gsw_desc: 'Define one standard-switch spec (bridge name / bond mode / IP mode), pick a set of hosts and choose uplink NICs per host. The platform rolls it out to each host for real, then lets you verify landing & consistency.',
    gsw_hosts: 'Target hosts',
    gsw_mode_ab: 'active-backup',
    gsw_ip_none: 'No IP (forwarding only, recommended)',
    gsw_need_host: 'Pick at least one target host',
    gsw_need_uplink_each: 'Each picked host needs at least one uplink NIC',
    gsw_mgmt_warn: 'Selected uplink includes the management NIC; rollout may drop this host',
    gsw_ack_mgmt: 'I understand: this host uplink includes the management NIC and may briefly disconnect; roll out anyway',
    gsw_no_free_nic: 'No free NIC available on this host (or unreachable)',
    gsw_apply: 'Apply',
    gsw_delete: 'Delete',
    gsw_check: 'Check consistency',
    gsw_apply_done: 'Rollout done: {ok}/{total} hosts succeeded',
    gsw_del_done: 'Delete done: {ok}/{total} hosts succeeded',
    gsw_del_confirm: 'Delete global switch "{name}" on selected hosts? This tears down the bridge/bond.',
    gsw_present: 'present',
    gsw_absent: 'absent',
    gsw_is_mgmt: 'carries mgmt traffic',
    gsw_consistency: 'Switch "{name}": {present}/{total} hosts landed',
    gsw_all_consistent: 'all consistent ✓',
    gsw_inconsistent: 'inconsistent — check hosts not landed',
    vmk_open: 'Service Interface',
    vmk_title: 'Service Interfaces · Traffic Labels',
    vmk_desc: 'A service interface is a host-level IP-bearing network interface; traffic labels separate service networks (management/migration/storage/FT/replication/provisioning). Realized as an IP bridge + firewalld zone; pick an uplink or host-only IP.',
    vmk_existing: 'Existing service interfaces',
    vmk_none: 'No service interface on this host yet',
    vmk_new: 'New service interface',
    vmk_all_used: 'All six traffic labels are created (one per label per host)',
    vmk_name: 'Name',
    vmk_role: 'Traffic label',
    vmk_ip: 'IP',
    vmk_uplink: 'Uplink',
    vmk_zone: 'Zone',
    vmk_ip_mode: 'IP mode',
    vmk_static: 'Static',
    vmk_ipv4: 'IPv4 address',
    vmk_prefix: 'Prefix length',
    vmk_gateway: 'Gateway',
    vmk_no_uplink: 'No uplink (host-only IP)',
    vmk_is_mgmt: 'carries mgmt traffic',
    vmk_mgmt_warn: 'This service interface touches the management network; the operation may affect host connectivity',
    vmk_ack_mgmt: 'I understand: management network involved, may briefly disconnect; proceed anyway',
    vmk_prefix_invalid: 'Prefix length must be 1..32',
    vmk_create: 'Create service interface',
    vmk_delete: 'Delete',
    vmk_created: 'Service interface created',
    vmk_deleted: 'Service interface deleted',
    vmk_del_confirm: 'Delete service interface "{name}"? This tears down its bridge and uplink.',
    // ===== P6 · VLAN (access port group + trunk) =====
    vld_open: 'VLAN', vld_title: 'VLAN Management (Port Group + Trunk)',
    vld_desc: 'Configure VLANs on standard switches: an access port group creates a VLAN sub-interface on a parent device bridged into a dedicated forwarding bridge (egress tags, ingress strips); a trunk enables VLAN filtering on a bridge to pass a set of tagged VLANs. All applied via real nmcli/bridge commands.',
    vld_access_existing: 'Existing access port groups', vld_trunk_existing: 'VLAN-filtered bridges (trunk)',
    vld_port_name: 'Port-group bridge', vld_vlan_id: 'VLAN ID', vld_parent: 'Parent',
    vld_allowed_vlans: 'Allowed VLANs', vld_no_access: 'No access port groups yet', vld_no_trunk: 'No trunk bridges yet',
    vld_new: 'New VLAN', vld_mode_access: 'Access port group', vld_mode_trunk: 'Trunk',
    vld_access_hint: 'Create an access port group for a VLAN on a parent device (bond/bridge). VMs attached to it sit in that VLAN.',
    vld_trunk_hint: 'Enable VLAN filtering on a bridge and allow a set of VLANs to pass tagged (trunk); VMs then set their own access VLAN.',
    vld_bridge_name: 'Port-group bridge name', vld_optional: 'optional', vld_no_parent: 'No parent device yet; create a standard switch first',
    vld_trunk_bridge: 'Target bridge', vld_vlans_hint: 'comma-separated, ranges allowed',
    vld_create: 'Create port group', vld_set_trunk: 'Apply trunk', vld_delete: 'Delete',
    vld_mgmt_warn: 'This device carries management traffic; VLAN changes may disrupt the management network (SSH may drop).',
    vld_ack_mgmt: 'I understand the risk, continue',
    vld_vlan_invalid: 'Invalid VLAN ID (valid range 1-4094)',
    vld_created: 'Access VLAN port group created', vld_deleted: 'VLAN port group deleted',
    vld_trunk_done: 'Trunk (VLAN) configured', vld_del_confirm: 'Delete this VLAN port group?',
    ...
    fw_short: 'Firewall', fw_manage: 'Manage firewall', fw_title: 'Firewall Management (firewalld)', fw_loading: 'Reading firewall state…',
    fw_load_fail: 'Cannot read firewall state (no credential or host unreachable)', fw_not_installed: 'firewalld is not installed on the host',
    fw_status: 'Firewall', fw_on: 'On', fw_off: 'Off', fw_turn_on: 'Enable firewall', fw_turn_off: 'Disable firewall',
    fw_platform: 'Platform ports', fw_ready: 'Ready', fw_not_ready: 'Not ready', fw_missing: 'Missing', fw_open_platform: 'Open platform ports',
    fw_open_ports: 'Open ports', fw_no_ports: 'No open ports', fw_services: 'Services', fw_remove: 'Remove',
    fw_add_custom: 'Add custom port policy', fw_port_ph: 'e.g. 8080 or 6000-6010', fw_add: 'Add', fw_port_invalid: 'Invalid port (e.g. 8080 or 6000-6010)',
    fw_op_done: 'Firewall change applied', fw_exec_steps: 'Executed commands', fw_batch_btn: 'Batch firewall', fw_batch_title: 'Batch Firewall (multi-host)',
    fw_batch_hint: 'Pick hosts and an action; runs concurrently and really on each host (per-host result)', fw_batch_action: 'Action', fw_batch_hosts: 'Select hosts',
    fw_act_open_platform: 'Open platform ports', fw_act_enable: 'Enable firewall (+platform ports)', fw_act_disable: 'Disable firewall',
    fw_act_add: 'Add ports', fw_act_remove: 'Remove ports', fw_batch_run: 'Run batch', fw_batch_pick: 'Select at least one host',
    fw_batch_done: 'Batch done: {ok}/{total} succeeded', fw_batch_results: 'Results', fw_batch_results_n: 'results',
    se_short: 'SELinux', se_manage: 'Manage SELinux', se_title: 'SELinux Management', se_loading: 'Reading SELinux state…',
    se_load_fail: 'Cannot read SELinux state (no credential or host unreachable)', se_not_avail: 'SELinux not supported on host',
    se_current: 'Runtime', se_persistent: 'Persistent', se_inconsistent: 'Runtime differs from persistent config (persistent wins after reboot)',
    se_set_mode: 'Set mode', se_reboot_note: 'Note: enforcing↔permissive applies instantly; switching to/from disabled needs a reboot to fully take effect (never faked).',
    se_need_reboot: 'This change requires a reboot to fully take effect', se_need_reboot_short: 'reboot', se_apply: 'Apply', se_op_done: 'SELinux set',
    se_batch_btn: 'Batch SELinux', se_batch_title: 'Batch SELinux (multi-host)', se_batch_hint: 'Pick hosts and a mode; runs really & concurrently (per-host result, reboot flagged)',
    se_batch_run: 'Run batch', se_batch_pick: 'Select at least one host', se_batch_done: 'Done: {ok}/{total} ok, {reboot} need reboot',
    sp_short: 'SSH Port', sp_manage: 'Change SSH port', sp_title: 'Change SSH Port', sp_batch_btn: 'Batch SSH port', sp_batch_title: 'Batch Change SSH Port',
    sp_risk: 'High-risk: a wrong port may lock you out of SSH. The platform keeps both old & new ports, verifies the new port, and only then switches over and syncs the DB. On failure the old port is kept, so the host is never lost.',
    sp_batch_risk: 'High-risk batch: changes sshd port on each selected host. Each host is verified independently; failures keep the old port and never affect other hosts.',
    sp_current: 'Current port', sp_new_port: 'New port (1-65535)', sp_safe_note: 'Auto-linked: SELinux port label + firewalld allow + dual-write sshd_config check + restart sshd + verify connectivity, then sync DB.',
    sp_ack: 'I understand the risk and confirm the port change', sp_need_ack: 'Please confirm the risk first', sp_invalid: 'Port must be an integer 1-65535',
    sp_apply: 'Change & verify', sp_verified: 'New port verified, DB synced', sp_verify_fail: 'New port verification failed, old port kept (DB unchanged)', sp_op_done: 'SSH port operation done',
    sp_batch_run: 'Run batch', sp_batch_pick: 'Select at least one host', sp_batch_done: 'Done: {ok}/{total} ok & verified',
