...
const DashboardView = {
  ...
  setup(props) {
    ...
    // GPU 概要：已用/空闲数 + 平均利用率（来自实时 metrics）
    const gpuSummary = computed(() => {
      const list = metrics.value.gpus || []
      if (!list.length) return { total: 0, busy: 0, idle: 0, avgUtil: 0 }
      const busy = list.filter((g) => g.util > 5).length
      const avgUtil = Math.round(list.reduce((s, g) => s + g.util, 0) / list.length)
    ...
    return { summary, metrics, clusters, tasks, rules, cpuColor, pct, sevColor, gpuSummary, clusterLoad, props, t }
  },
  template: `
    <div>
      <!-- ===== overview：资源概览 ===== -->
      <template v-if="props.tab==='overview'">
        ...
        <div class="grid grid-2" style="margin-top:20px">
          ...
          <div class="apple-card">
            ...
            <div class="dash-gpu-stats" style="margin-top:14px">
              <div class="dgs-item"><div class="dgs-num">{{ gpuSummary.total }}</div><div class="muted">{{ t('dash_gpu_total') }}</div></div>
              <div class="dgs-item"><div class="dgs-num" style="color:var(--color-green)">{{ gpuSummary.busy }}</div><div class="muted">{{ t('dash_gpu_busy') }}</div></div>
              <div class="dgs-item"><div class="dgs-num" style="color:var(--text-tertiary)">{{ gpuSummary.idle }}</div><div class="muted">{{ t('dash_gpu_idle') }}</div></div>
            ...
            <div style="margin-top:16px">
              <div v-for="g in metrics.gpus" :key="g.id" class="flex between" style="margin-bottom:8px;gap:10px">
                <span class="muted mono" style="font-size:12px;min-width:48px">GPU{{ g.id }}</span>
                <div class="usage-bar" style="flex:1"><div class="fill" :style="{width:g.util+'%',background:cpuColor(g.util)}"></div></div>
                <span class="mono" style="font-size:12px;min-width:40px;text-align:right">{{ Math.round(g.util) }}%</span>
              </div>
            </div>
          </div>
        </div>
